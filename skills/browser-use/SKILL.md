...
## Browser Usage Rules (Enforced)

Before every invocation of the browser tool (action: "open"), **you must** first perform the following check flow. There are no exceptions to this rule. You must not rely on memory or previous check results from the context—every invocation requires a fresh check.

### 1. Detect if Browser is Running

Check if the browser is running by verifying if local port 9222 is being listened on:
```bash
lsof -i :9222 || netstat -tlnp 2>/dev/null | grep 9222 || ss -tlnp | grep 9222
```

- **If port 9222 is listening**: Browser is running, use the browser tool directly.
- **If port 9222 is not listening**: Browser is not running, you must first execute the startup flow (see below), wait for the browser to be ready, then use the browser tool.

### 2. Start the Browser

When detecting that the browser is not running, execute the following command to start the browser:
```bash
bash /opt/browser-vnc/start-chromium.sh
```

### 3. Handling Browser Service Unavailability

If you encounter connection failures, timeouts, or any errors indicating that the browser service is unavailable while using the browser tool, **do not retry the browser operation directly**. Instead:

1. Re-execute step 1 to check port 9222 status.
2. If the port is not listening, execute step 2 to restart the browser.
3. After the browser is ready, retry the previously failed browser operation.

### Key Principles

- **No Memory Dependence**: Even if you just checked the browser status in the previous step, you must still re-detect before the next browser tool invocation. Every invocation is independent.
- **Detect Before Operation**: Never assume the browser is running—always confirm with port detection.
- **Error = Re-check**: Any browser-related error should trigger the complete detect → start → retry flow.
